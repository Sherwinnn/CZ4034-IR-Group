/**
 * forked from Solr-Node on 2018.8.1 by Seech
 */
...
const logger =  require('log4js').getLogger('nolr');
...
function Client(options) {
  ...
  this.MLT_PATH = 'mlt';
  this.UPDATE_JSON_SINGLE_DOC_PATH = 'update/json/docs';
  this.UPDATE_JSON_ARRAY_PATH = 'update/json';
 ...
 * @returns {Undefined|Promise}
 */
Client.prototype.updateMany = function(data, options, finalCallback) {
  var bodyData;
  ...
  if (typeof finalCallback === 'function') {
    this._requestPost(this.UPDATE_JSON_ARRAY_PATH, bodyData, options, finalCallback);
  } else {
    return this._requestPost(this.UPDATE_JSON_ARRAY_PATH, bodyData, options);
  }
};
 ...
 * @returns {Undefined|Promise}
 */
Client.prototype.updateOne = function(data, options, finalCallback) {
  var bodyData = data;
  if (arguments.length < 3 && _.isFunction(options)) {
    finalCallback = options;
    options = { wt: 'json', commit: true }; // 'commit:true' option is default
  }
  if (typeof finalCallback === 'function') {
    this._requestPost(this.UPDATE_JSON_SINGLE_DOC_PATH, bodyData, options, finalCallback);
  } else {
    return this._requestPost(this.UPDATE_JSON_SINGLE_DOC_PATH, bodyData, options);
  }
};
...
Client.prototype.commit = function(finalCallback) {
  if (typeof finalCallback === 'function') {
    this._requestPost(this.UPDATE_JSON_SINGLE_DOC_PATH, {}, {commit: true}, finalCallback);
  } else {
    return this._requestPost(this.UPDATE_JSON_SINGLE_DOC_PATH, {}, {commit: true});
  }
};
...
Client.prototype.softCommit = function(finalCallback) {
  if (typeof finalCallback === 'function') {
    this._requestPost(this.UPDATE_JSON_SINGLE_DOC_PATH, {}, {softCommit: true}, finalCallback);
  } else {
    return this._requestPost(this.UPDATE_JSON_SINGLE_DOC_PATH, {}, {softCommit: true});
  }
};
